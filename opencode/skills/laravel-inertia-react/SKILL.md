...
**WAJIB**: Semua TypeScript interface dan type **harus** disimpan di folder `resources/js/types/` — tidak boleh inline di dalam file page atau component.

- `resources/js/types/index.d.ts` — shared global types (`PageProps`, `User`, `PaginatedData`, dll.)
- `resources/js/types/<domain>.d.ts` — satu file per domain/fitur (contoh: `post.d.ts`, `category.d.ts`)
- Import menggunakan alias `@/types` atau `@/types/<file>`

Interface props halaman (`interface XxxProps extends PageProps`) boleh didefinisikan di dalam file page karena bersifat lokal untuk halaman tersebut, tapi model/entity types **harus** di folder types.
...
- `form-component` - Use Form component for declarative form submissions
...
- `form-optimistic-updates` - Update UI instantly with automatic rollback on failure
...
- `nav-instant-visits` - Render target page immediately while server responds
...
All TypeScript interfaces and types **must** be stored in `resources/js/types/` — never inline inside page or component files.

```
resources/js/types/
├── index.d.ts        # Global shared types (PageProps, User, etc.)
├── post.d.ts         # Post-related types
├── category.d.ts     # Category-related types
└── ...               # One file per domain/feature
```

Types are then imported from `@/types` or `@/types/<file>`.
...
```tsx
// resources/js/types/category.d.ts
...
### Laravel + Inertia React App Structure (resources/js)

```
resources/js/
├── types/
│   ├── index.d.ts           # PageProps, User, PaginatedData, shared types
│   ├── post.d.ts            # Post entity types
│   ├── category.d.ts        # Category entity types
│   └── <domain>.d.ts        # One file per domain/feature
├── Pages/
│   └── Posts/
│       ├── Index.tsx        # Import types from @/types/post
│       └── Create.tsx       # Import types from @/types/category
├── Layouts/
│   └── AppLayout.tsx
└── Components/
```
